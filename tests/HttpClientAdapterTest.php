...
class HttpClientAdapterTest extends \PHPUnit_Framework_TestCase {
    ...
    public function setUp() {
        ...
        $this->request = Phake::partialMock('WyriHaximus\React\RingPHP\HttpClient\Request',
			$this->requestArray,
			$this->httpClient,
            $this->loop
        );
    ...
    public function testSend() {
		Phake::when($this->requestFactory)->create($this->requestArray, $this->httpClient, $this->loop)->thenReturn($this->request);

        $adapter = $this->adapter;
        $adapter($this->requestArray);

		Phake::inOrder(
			Phake::verify($this->requestFactory, Phake::times(1))->create($this->requestArray, $this->httpClient, $this->loop),
    ...
    public function testSetRequestFactory() {
        ...
        $this->assertInstanceOf('WyriHaximus\React\RingPHP\HttpClient\RequestFactory', $this->adapter->getRequestFactory());
